...
const cycleDay = (startDate) => {
  if (!startDate) return 1;
  const diff = daysBetween(startDate);
  return (diff % 28) + 1;
};
const ls = {
...
function WeekCalendar({ periodStart, currentDay, loggedDays, onDayPress }) {
  ...
  const days = Array.from({ length: 7 }, (_, i) => {
    ...
    const diffFromPeriod = periodStart
      ? Math.floor((d - new Date(periodStart)) / 86400000)
      : null;
    const cDay = diffFromPeriod !== null ? (diffFromPeriod % 28) + 1 : null;
    const phIdx = cDay === null ? null : cDay <= 5 ? 0 : cDay <= 13 ? 1 : cDay <= 16 ? 2 : 3;
    const isToday = d.toDateString() === todayDate.toDateString();
    const dateKey = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
    const hasLog = !!loggedDays[cDay];
    return { date: d, dow: DOW[d.getDay()], dayNum: d.getDate(), cDay, phIdx, isToday, dateKey, hasLog };
  });

  return (
    <div style={{ display: "flex", gap: 5 }}>
      {days.map((d, i) => {
        const ph = d.phIdx !== null ? phases[d.phIdx] : null;
        return (
          <button key={i} onClick={() => d.cDay && onDayPress(d.cDay)}
            style={{
              flex: 1, borderRadius: 14, padding: "8px 2px",
              background: d.isToday ? (ph ? ph.color : C.peach) : (ph ? ph.soft : C.creamDark),
              border: d.isToday ? `2px solid ${ph ? ph.color : C.peach}` : `1px solid ${ph ? ph.mid : C.sand}`,
              cursor: d.cDay ? "pointer" : "default",
              display: "flex", flexDirection: "column", alignItems: "center", gap: 3,
            }}>
            <div style={{ fontSize: 9, color: d.isToday ? C.white : C.textLight, fontWeight: 600, textTransform: "uppercase" }}>{d.dow}</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: d.isToday ? C.white : C.text }}>{d.dayNum}</div>
            <div style={{ fontSize: 9 }}>{ph ? ph.emoji : "·"}</div>
            {d.cDay && <div style={{ fontSize: 8, color: d.isToday ? "rgba(255,255,255,0.8)" : C.textLight }}>D{d.cDay}</div>}
            {d.hasLog && <div style={{ width: 4, height: 4, borderRadius: "50%", background: d.isToday ? C.white : (ph ? ph.color : C.peach) }} />}
          </button>
        );
      })}
    </div>
...
export default function FemFlow() {
  ...
  const [periodStart, setPeriodStart] = useState(() => ls.get("ff_periodStart", null));
  const [loggedDays, setLoggedDays] = useState(() => ls.get("ff_logs", {}));
  const [checkedGrocery, setCheckedGrocery] = useState(() => ls.get("ff_grocery", {}));
  ...
  const [showSetup, setShowSetup] = useState(!ls.get("ff_periodStart", null));

  // Persist to localStorage on every change
  useEffect(() => { ls.set("ff_periodStart", periodStart); }, [periodStart]);
  useEffect(() => { ls.set("ff_logs", loggedDays); }, [loggedDays]);
  useEffect(() => { ls.set("ff_grocery", checkedGrocery); }, [checkedGrocery]);

  const currentDay = periodStart ? cycleDay(periodStart) : 1;
  const phaseIdx = currentDay <= 5 ? 0 : currentDay <= 13 ? 1 : currentDay <= 16 ? 2 : 3;
  const phase = phases[phaseIdx];

  const saveLog = (day, log) => setLoggedDays(p => ({ ...p, [day]: log }));

  /* ── ONBOARDING ── */
  if (showSetup) {
    return (
      <div style={{ fontFamily: "Georgia,serif", background: C.cream, minHeight: "100vh", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32 }}>
        ...
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 32, fontWeight: 700, color: C.text, textAlign: "center", marginBottom: 8 }}>Welcome to FemFlow</div>
        <div style={{ fontSize: 14, color: C.textSoft, textAlign: "center", lineHeight: 1.7, marginBottom: 36 }}>
          To get started, tell us when your last period began. FemFlow will automatically track where you are in your cycle every day.
        </div>
        <div style={{ width: "100%", background: C.white, borderRadius: 20, padding: 24, boxShadow: "0 4px 24px rgba(74,55,40,0.1)" }}>
          <div style={{ fontSize: 11, color: C.textLight, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 10 }}>When did your last period start?</div>
          <input type="date" max={today()}
            defaultValue={today()}
            id="periodInput"
            style={{ width: "100%", padding: "14px", borderRadius: 12, border: `1.5px solid ${C.sand}`, fontSize: 15, color: C.text, background: C.cream, outline: "none", boxSizing: "border-box", marginBottom: 16 }}
          />
          <button onClick={() => {
            const val = document.getElementById("periodInput").value;
            if (val) { setPeriodStart(val); setShowSetup(false); }
          }}
            style={{ width: "100%", background: C.peach, border: "none", borderRadius: 14, padding: "15px", fontSize: 14, fontWeight: 700, color: C.white, cursor: "pointer", fontFamily: "'Cormorant Garamond',serif", letterSpacing: "0.5px" }}>
            Start Tracking 🌸
          </button>
        </div>
        <div style={{ fontSize: 11, color: C.textLight, marginTop: 16, textAlign: "center", lineHeight: 1.6 }}>
  ...
  const HomePage = () => {
    ...
    const openLog = (d) => { setLocalLog(loggedDays[d] || { flow: "None", symptoms: [], mood: "" }); setLogDay(d); };
    ...
    return (
      <div style={{ paddingBottom: 90 }}>
        <div style={{ background: `linear-gradient(150deg,${phase.soft} 0%,${C.cream} 80%)`, padding: "36px 20px 24px", borderBottom: `1px solid ${phase.mid}` }}>
          ...
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 30, color: C.text, fontWeight: 700, lineHeight: 1.1, marginBottom: 4 }}>{phase.emoji} {phase.name} Phase</div>
          <div style={{ fontSize: 13, color: C.textSoft, fontStyle: "italic", marginBottom: 18 }}>{phase.tagline}</div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <span style={{ background: phase.color, color: C.white, borderRadius: 20, padding: "5px 14px", fontSize: 11, fontWeight: 700 }}>Day {currentDay} of 28</span>
        ...
        <div style={{ padding: "20px 16px 0" }}>
          ...
          <div style={{ background: C.white, borderRadius: 18, padding: "18px", marginBottom: 14, boxShadow: "0 2px 14px rgba(74,55,40,0.07)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ fontSize: 10, color: C.textLight, letterSpacing: "1.5px", textTransform: "uppercase" }}>This Week</div>
              <button onClick={() => setShowSetup(true)} style={{ background: "none", border: "none", fontSize: 11, color: C.peach, cursor: "pointer", fontWeight: 600 }}>Update period date</button>
            </div>
            <WeekCalendar periodStart={periodStart} currentDay={currentDay} loggedDays={loggedDays} onDayPress={openLog} />
            <div style={{ marginTop: 10, fontSize: 11, color: C.textSoft, textAlign: "center", fontStyle: "italic" }}>
              Tap any day to log symptoms · Automatically advances each day
            </div>
          </div>

          {/* Log inline */}
          {logDay && (
            <div style={{ background: C.white, borderRadius: 18, padding: 20, marginBottom: 14, boxShadow: "0 4px 24px rgba(74,55,40,0.1)", border: `1px solid ${C.sand}` }}>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 700, color: C.text, marginBottom: 14 }}>📝 Log — Cycle Day {logDay}</div>
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 10, color: C.textLight, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 8 }}>Flow</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {flows.map(f => <button key={f} onClick={() => setLocalLog(p => ({ ...p, flow: f }))} style={{ background: localLog.flow === f ? C.peach : C.creamDark, border: `1px solid ${localLog.flow === f ? C.peach : C.sand}`, borderRadius: 20, padding: "7px 12px", fontSize: 12, cursor: "pointer", color: localLog.flow === f ? C.white : C.text, fontWeight: localLog.flow === f ? 700 : 400 }}>{f}</button>)}
                </div>
              </div>
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 10, color: C.textLight, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 8 }}>Mood</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {moods.map(m => <button key={m} onClick={() => setLocalLog(p => ({ ...p, mood: m }))} style={{ background: localLog.mood === m ? C.teal : C.creamDark, border: `1px solid ${localLog.mood === m ? C.teal : C.sand}`, borderRadius: 20, padding: "6px 10px", fontSize: 11, cursor: "pointer", color: localLog.mood === m ? C.white : C.text, fontWeight: localLog.mood === m ? 700 : 400 }}>{m}</button>)}
                </div>
              </div>
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 10, color: C.textLight, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 8 }}>Symptoms</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {symptoms.map(s => <button key={s} onClick={() => togSym(s)} style={{ background: localLog.symptoms.includes(s) ? C.green : C.creamDark, border: `1px solid ${localLog.symptoms.includes(s) ? C.green : C.sand}`, borderRadius: 20, padding: "6px 10px", fontSize: 11, cursor: "pointer", color: localLog.symptoms.includes(s) ? C.white : C.text, fontWeight: localLog.symptoms.includes(s) ? 700 : 400 }}>{s}</button>)}
                </div>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => { saveLog(logDay, localLog); setLogDay(null); }} style={{ flex: 1, background: C.peach, border: "none", borderRadius: 14, padding: "13px", fontSize: 13, fontWeight: 700, color: C.white, cursor: "pointer" }}>Save ✓</button>
                <button onClick={() => setLogDay(null)} style={{ background: C.creamDark, border: "none", borderRadius: 14, padding: "13px 18px", fontSize: 13, color: C.textSoft, cursor: "pointer" }}>Cancel</button>
              </div>
            </div>
          )}
            ...
            {phase.dailySchedule.map((slot, i) => (
              <div key={i} style={{ display: "flex", gap: 12 }}>
                ...
                <div style={{ flex: 1, paddingBottom: i < phase.dailySchedule.length - 1 ? 18 : 0 }}>
                  <div style={{ fontSize: 10, color: C.textLight, marginBottom: 1 }}>{slot.time}</div>
                  <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 15, fontWeight: 700, color: C.text, marginBottom: 4 }}>{slot.title}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[{ label: "Today's Workout", icon: "🏋️", sub: "View your split", page: "fitness" }, { label: "Meal Plan", icon: "🥗", sub: "Today's meals", page: "meals" }, { label: "Log Symptoms", icon: "📝", sub: "Track your cycle", page: "cycle" }, { label: "Grocery List", icon: "🛒", sub: "Shop by phase", page: "meals" }].map(item => (
              <button key={item.label} onClick={() => { setNav(item.page); if (item.label === "Grocery List") setMealTab("Grocery List"); }}
                style={{ background: C.white, border: `1px solid ${phase.mid}`, borderRadius: 16, padding: "16px 14px", cursor: "pointer", textAlign: "left", boxShadow: "0 2px 10px rgba(74,55,40,0.06)" }}>
  ...
  const CyclePage = () => {
    ...
    const [localLog, setLocalLog] = useState({ flow: "None", symptoms: [], mood: "" });
    const open = d => { setLocalLog(loggedDays[d] || { flow: "None", symptoms: [], mood: "" }); setActiveLog(d); };
    const togSym = s => setLocalLog(p => ({ ...p, symptoms: p.symptoms.includes(s) ? p.symptoms.filter(x => x !== s) : [...p.symptoms, s] }));
    ...
    return (
      <div style={{ paddingBottom: 90 }}>
        <div style={{ background: `linear-gradient(150deg,${C.peachSoft} 0%,${C.cream} 80%)`, padding: "36px 20px 20px", borderBottom: `1px solid ${C.peachMid}` }}>
          ...
          <div style={{ fontSize: 13, color: C.textSoft, fontStyle: "italic" }}>Day {currentDay} of 28 · {phase.name} Phase</div>
        </div>
        <div style={{ padding: "20px 16px" }}>

          {/* Update period start */}
          <div style={{ background: C.white, borderRadius: 16, padding: "16px 18px", marginBottom: 14, boxShadow: "0 2px 10px rgba(74,55,40,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 10, color: C.textLight, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 3 }}>Period Started</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{periodStart ? new Date(periodStart).toLocaleDateString("en-US", { month: "long", day: "numeric" }) : "Not set"}</div>
            </div>
            <button onClick={() => setShowSetup(true)} style={{ background: C.peachSoft, border: `1px solid ${C.peachMid}`, borderRadius: 12, padding: "8px 14px", fontSize: 11, fontWeight: 700, color: C.peach, cursor: "pointer" }}>Update</button>
          </div>
          ...
          {/* Full 28-day calendar */}
          <div style={{ background: C.white, borderRadius: 18, padding: "16px", marginBottom: 14, boxShadow: "0 2px 14px rgba(74,55,40,0.07)" }}>
            <div style={{ fontSize: 10, color: C.textLight, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 12 }}>Full Cycle — Tap any day to log</div>
          ...
          {activeLog && (
            <div style={{ background: C.white, borderRadius: 18, padding: 20, marginBottom: 16, boxShadow: "0 4px 24px rgba(74,55,40,0.1)", border: `1px solid ${C.sand}` }}>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, fontWeight: 700, color: C.text, marginBottom: 14 }}>📝 Day {activeLog}</div>
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 10, color: C.textLight, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 8 }}>Flow</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {flows.map(f => <button key={f} onClick={() => setLocalLog(p => ({ ...p, flow: f }))} style={{ background: localLog.flow === f ? C.peach : C.creamDark, border: `1px solid ${localLog.flow === f ? C.peach : C.sand}`, borderRadius: 20, padding: "7px 12px", fontSize: 12, cursor: "pointer", color: localLog.flow === f ? C.white : C.text, fontWeight: localLog.flow === f ? 700 : 400 }}>{f}</button>)}
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => { saveLog(activeLog, localLog); setActiveLog(null); }} style={{ flex: 1, background: C.peach, border: "none", borderRadius: 14, padding: "13px", fontSize: 13, fontWeight: 700, color: C.white, cursor: "pointer" }}>Save ✓</button>
              ...
              {Object.entries(loggedDays).sort((a, b) => Number(b[0]) - Number(a[0])).map(([day, log]) => (
                <div key={day} style={{ background: C.white, borderRadius: 14, padding: "14px 16px", marginBottom: 10, boxShadow: "0 1px 8px rgba(74,55,40,0.06)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 13, fontWeight: 700, color: C.text }}>Day {day}</div>
  ...
  const MealsPage = () => (
    <div style={{ paddingBottom: 90 }}>
      ...
      <div style={{ padding: "20px 16px" }}>
        {mealTab === "Meals" && (
          <>
            <div style={{ fontSize: 13, color: C.textSoft, marginBottom: 16, lineHeight: 1.7, fontStyle: "italic" }}>Every meal chosen to work with your {phase.name} phase hormones — nourishing your body from the inside out.</div>
  ...
  const NavBtn = ({ id, icon, label }) => {
    ...
    return (
      <button onClick={() => setNav(id)} style={{ flex: 1, background: "none", border: "none", padding: "10px 4px 6px", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
        ...
        <div style={{ fontSize: 9, fontWeight: on ? 700 : 400, color: on ? C.peach : C.textLight, letterSpacing: "0.5px", textTransform: "uppercase" }}>{label}</div>
